        ...
        <section id="contact" className="py-20 md:py-28 bg-gradient-to-br from-slate-50 to-gray-500">
          <div className="container">
            <div className="text-center mb-14">
              <h1 className="text-3xl md:text-4xl font-bold">{text("찾아오시는 길", "Directions")}</h1>
              <div className="divider-modern mx-auto w-24 mb-6" />
              <p className="mt-3 text-muted-foreground">{text("오시는 길 및 연락처 안내", "Location and contact information")}</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
              {/* 주소/연락처 */}
              <div className="rounded-2xl border p-8 bg-background h-full">
                <h2 className="text-xl font-semibold mb-6">{text("회사 정보", "Company Information")}</h2>

                <div className="space-y-4 text-sm">
                  <div>
                    <div className="font-semibold">{text("주소", "Address")}</div>
                    <div className="text-muted-foreground mt-1">
                      {text("경남 창원시 성산구 완암로 147", "147 Wanam-ro, Seongsan-gu, Changwon-si, Gyeongnam, Korea")}
                    </div>
                  </div>

                  <div>
                    <div className="font-semibold">{text("연락처", "Phone")}</div>
                    <div className="text-muted-foreground mt-1">055-288-0856</div>
                  </div>

                  <div>
                    <div className="font-semibold">{text("팩스", "Fax")}</div>
                    <div className="text-muted-foreground mt-1">055-288-0857</div>
                  </div>

                  <div>
                    <div className="font-semibold">{text("이메일", "Email")}</div>
